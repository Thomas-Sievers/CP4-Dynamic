...
matplotlib.use("Agg")  # headless backend: tests only save/inspect figures, never display them
...
def _real_instance_pipeline():
    """Shared plumbing for the plotting smoke tests: real seed-1 instance, pi order, distances."""
    instance = generate_instance(seed=data_generator.SEED)
    graph = instance.graph()
    depot_id = instance.depot.node_id
    distance_from_depot, parent = dijkstra(graph, depot_id)
    pi_order = build_order(distance_from_depot, parent, depot_id)
    distance = distance_matrix(graph, [depot_id, *pi_order])
    return instance, pi_order, distance


def test_plot_instance_graph_returns_a_figure():
    # Figure 1 must render without error on the real dataset.
    instance, _, _ = _real_instance_pipeline()
    fig = plot_instance_graph(instance)
    assert isinstance(fig, plt.Figure)
    plt.close(fig)


def test_plot_solution_returns_a_figure():
    # Figure 2 must render without error for a real Solution.
    instance, pi_order, distance = _real_instance_pipeline()
    capacity = int(0.35 * sum(site.load for site in instance.sites))
    solution = solve_greedy(instance, capacity=capacity, pi_order=pi_order, distance=distance)
    fig = plot_solution(instance, solution)
    assert isinstance(fig, plt.Figure)
    plt.close(fig)


def test_plot_dp_table_returns_a_figure():
    # Figure 3 must render without error, including UNREACHABLE cells.
    instance, pi_order, distance = _real_instance_pipeline()
    capacity = int(0.35 * sum(site.load for site in instance.sites))
    solution, dp = solve_dp(instance, capacity=capacity, pi_order=pi_order, distance=distance)
    fig = plot_dp_table(instance, dp, pi_order, solution)
    assert isinstance(fig, plt.Figure)
    plt.close(fig)


def test_generate_all_figures_writes_the_three_pngs(tmp_path):
    # The Part E deliverable: three PNG files, generated by code, not hand-drawn.
